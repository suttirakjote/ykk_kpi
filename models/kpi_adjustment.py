...
class KpiAdjustment(models.Model):
    ...
    def action_populate_employees(self):
        ...
        # default New Grade = Current Grade สำหรับบรรทัดที่ยังไม่ได้ปรับ
        for line in self.line_ids.filtered(lambda l: not l.new_grade):
...
class KpiAdjustmentLine(models.Model):
    ...
    # New Grade = เกรดใหม่ที่ถูกปรับ (plain field แก้ไขได้ - default ผ่าน populate/onchange)
    # ไม่ทำเป็น computed เพื่อกันการ recompute ทับค่าที่ผู้ใช้แก้ (bug ดีดกลับค่าเดิม)
    ...
    @api.onchange('department_kpi_id', 'employee_id')
    def _onchange_default_new_grade(self):
        """ตั้งค่า default New Grade = Current Grade เมื่อเพิ่มบรรทัดเอง (ถ้ายังว่าง)"""
        for line in self:
            if not line.new_grade and line.current_grade:
                line.new_grade = line.current_grade
