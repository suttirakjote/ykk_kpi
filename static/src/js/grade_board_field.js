...
export class KpiGradeBoardField extends Component {
    ...
    setup() {
        ...
        // ข้อมูลเสริมบนการ์ด: Start Work Date / คะแนนประเมิน / Parent KPI grades
        this.cardState = useState({ startByEmp: {}, scoreByDept: {}, parentsByDept: {} });
        onWillStart(async () => {
    ...
    // ----- CARD EXTRA DATA (Start Work Date / Score / Parent KPI grades) -----
    async _loadCardData() {
        const lines = this.lines;
        const empIds = [...new Set(lines.map((l) => l.data.employee_id && l.data.employee_id[0]).filter(Boolean))];
        const deptIds = [...new Set(lines.map((l) => l.data.department_kpi_id && l.data.department_kpi_id[0]).filter(Boolean))];
        const startByEmp = {};
        const scoreByDept = {};
        const parentsByDept = {};
        if (empIds.length) {
            const emps = await this.orm.read("hr.employee", empIds, ["ykk_start_work_date"]);
            for (const e of emps) {
                startByEmp[e.id] = e.ykk_start_work_date || "";
            }
        }
        if (deptIds.length) {
            const depts = await this.orm.read("ykk.kpi.department.kpi", deptIds, ["period_grade_id"]);
            for (const d of depts) {
                scoreByDept[d.id] = d.period_grade_id ? d.period_grade_id[1] : "";
            }
            const sums = await this.orm.searchRead(
                "ykk.kpi.department.kpi.summary.line",
                [["department_kpi_id", "in", deptIds]],
                ["department_kpi_id", "parent_kpi_id", "grade_id"]
            );
            for (const s of sums) {
                const dId = s.department_kpi_id[0];
                (parentsByDept[dId] = parentsByDept[dId] || []).push({
                    parent: s.parent_kpi_id ? s.parent_kpi_id[1] : "",
                    grade: s.grade_id ? s.grade_id[1] : "",
                });
            }
        }
        this.cardState.startByEmp = startByEmp;
        this.cardState.scoreByDept = scoreByDept;
        this.cardState.parentsByDept = parentsByDept;
    }

    cardStart(line) {
        const id = line.data.employee_id && line.data.employee_id[0];
        return id ? this.cardState.startByEmp[id] || "" : "";
    }

    cardScore(line) {
        const id = line.data.department_kpi_id && line.data.department_kpi_id[0];
        if (!id) {
            return "";
        }
        const v = this.cardState.scoreByDept[id];
        return v == null ? "" : v;
    }

    cardParents(line) {
        const id = line.data.department_kpi_id && line.data.department_kpi_id[0];
        return id ? this.cardState.parentsByDept[id] || [] : [];
    }
